...
function processLanguages(languages) {
  ...
  for (var language in languages) {
    // only languages no other object shit
    if(!languages.hasOwnProperty(language)) continue;

    const lines = languages[language];
    totalLines += lines;

    output.push({ name: language,  lines });
  }

  output.map(language => {
    language.percentage = (language.lines / totalLines) * 100;
    language.color = githubColours[language.name];
    return language;
  });
      ...
      events = events.map((event, i) => {
        ...
        switch (event.type) {
          ...
          case 'PushEvent':
            ...
            request({
              method: 'GET',
              url: `https://api.github.com/repos/${event.repo.name}/languages`,
              headers: {
                'User-Agent': 'zaccolley.com',
                'Authorization': `Basic ${process.env.GITHUB_BASIC_AUTH}`
              }
              ...
              output.languages = processLanguages(languages);
            });
